...
class RNASSDataGenerator(object):
    ...
    def load_data(self):
        ...
        self.data_x = np.array([instance[0] for instance in self.data])  # one_hot_matrix_600
        self.data_y = np.array([instance[1] for instance in self.data])  # ss_label_600
        self.pairs = np.array([instance[-1] for instance in self.data])  # pair_dict_all_list
        # pdb.set_trace()
        self.seq_length = np.array([instance[2] for instance in self.data])  # seq_len
...
class RNASSDataGenerator_input(object):
    ...
    def load_data(self):
        ...
        self.seq = [itm.strip().upper().replace('T', 'U') for itm in input_file if
                    itm.upper().startswith(('A', 'U', 'C', 'G', 'T'))]
    ...
    def one_hot_600(self, seq_item):
        ...
        feat = np.concatenate(
            [[(bases == base.upper()).astype(int)] if str(base).upper() in BASES else np.array([[-1] * len(BASES)]) for
             base
             in RNN_seq])
        if len(seq_item) <= 600:
...
class Dataset_Cut_8(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        for i in range(4):
            for j in range(i, 4):
                if i != j:
                    data_fcn[n_dim, :data_len, :data_len] = np.matmul(data_seq[:data_len, i].reshape(-1, 1),
                                                                      data_seq[:data_len, j].reshape(1,
                                                                                                     -1)) + np.matmul(
                        data_seq[:data_len, j].reshape(-1, 1), data_seq[:data_len, i].reshape(1, -1))
                else:
                    data_fcn[n_dim, :data_len, :data_len] = np.matmul(data_seq[:data_len, i].reshape(-1, 1),
                                                                      data_seq[:data_len, j].reshape(1, -1))
                n_dim += 1
...
class Dataset_Cut_concat_new_merge(data.Dataset):
    ...
    def merge_data(self):
        ...
        self.data2.seq_length = np.concatenate((self.data1.seq_length, self.data2.seq_length, self.data3.seq_length),
                                               axis=0)
        self.data2.pairs = np.concatenate((self.data1.pairs, self.data2.pairs, self.data3.pairs), axis=0)
        self.data2.data_name = np.concatenate((self.data1.data_name, self.data2.data_name, self.data3.data_name),
                                              axis=0)
